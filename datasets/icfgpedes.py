...
class ICFGPEDES(BaseDataset):
    ...
    def _process_anno(self, annos: List[dict], training=False):
        ...
        if training:
            ...
            # For test/val: we need to keep track of which caption comes from which image
            img_paths = []  # list of all unique image paths (gallery)
            image_pids = []  # pid for each image
            captions = []  # all captions (queries)
            caption_pids = []  # pid for each caption
            gt_img_paths = []  # ✅ the actual image path that each caption describes

            for anno in annos:
                pid = int(anno['id'])
                pid_container.add(pid)
                img_path = op.join(self.img_dir, anno['file_path'])  # this image
                caption_list = anno['captions']
                for caption in caption_list:
                    captions.append(caption)
                    caption_pids.append(pid)
                    gt_img_paths.append(img_path)  # ✅ every caption points to its source image

                # Also add this image to gallery
                img_paths.append(img_path)
                image_pids.append(pid)
            ...
            dataset = {
                ...
                "caption_pids": caption_pids,
                "captions": captions,
                "gt_img_paths": gt_img_paths,  # ✅ added!
            }
            return dataset, pid_container
