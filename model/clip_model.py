...
from model.moe import MoeLayer, MoeArgs
...
#         return out
        # return x
class Adapter(nn.Module):
    def __init__(self, c_in, c_out, reduction=8, dropout_rate=0.1, use_residual=True):
        super(Adapter, self).__init__()

        # 添加层归一化，提高训练稳定性
        self.norm = nn.LayerNorm(c_in,dtype=torch.float16)
...
class MOEResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None,
                 num_experts=6, topk=2, reduction=8, i=None):
        super().__init__()
        ...
        self.comb_param = nn.Parameter(torch.randn(d_model))

        if self.index >= 0:
            self.feed_forward = MoeLayer(
                experts=[Adapter(d_model, d_model, reduction, 0.1, True) for _ in range(num_experts)],
                gate_img=nn.Linear(d_model, num_experts, bias=False),
                gate_txt=nn.Linear(d_model, num_experts, bias=False),
                task_gate=nn.Linear(d_model, num_experts, bias=False),
                moe_args=MoeArgs(num_experts=num_experts, num_experts_per_tok=topk)
            )
    ...
    def forward(self, x: torch.Tensor, l_aux: torch.Tensor, is_text: bool):
        # 注意：现在 forward 不再接受 tuple，而是显式参数（更清晰）
        x = x + self.attention(self.ln_1(x))

        if self.index >= 0:
            y, l_aux_new = self.feed_forward(self.ln_3(x), self.comb_param.to(x.dtype), is_text=is_text)
            x = x + self.mlp(self.ln_2(x)) + y
            l_aux = l_aux + l_aux_new
        else:
...
class MOETransformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None,
                 num_experts=6, topk=2, reduction=8):
        super().__init__()
        self.width = width
        self.layers = layers
        # 使用 ModuleList 而非 Sequential，便于控制 forward
        self.resblocks = nn.ModuleList([
            MOEResidualAttentionBlock(width, heads, attn_mask, num_experts, topk, reduction, i)
            for i in range(layers)
        ])

    def forward(self, x: torch.Tensor, l_aux: torch.Tensor, is_text: bool):
        for block in self.resblocks:
            x, l_aux = block(x, l_aux, is_text=is_text)
        return x, l_aux
