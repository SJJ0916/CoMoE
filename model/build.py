...
class IRRA(nn.Module):
    def __init__(self, args, num_classes=11003):
        ...
        self.STransformer = MOETransformer(self.embed_dim, layers, heads, attn_mask=None, num_experts=args.num_experts, topk=args.topk, reduction=args.reduction)
        for i in range(layers):
            for j in range(args.num_experts):
                nn.init.kaiming_uniform_(self.STransformer.resblocks[i].feed_forward.experts[j].down.weight,
                                         a=math.sqrt(5))
    ...
    def encode_image_s(self, image):
        ...
        x = self.base_model.encode_image(image)
        i_feats = x[:, 0, :].float()
        S_image_feats,_ = self.STransformer(x[:, 1:, :],l_aux=0.0, is_text=False)
        S_image_feats = torch.mean(S_image_feats, dim=1)
        i_feats = S_image_feats + i_feats
        end_time = time.time()
        # print(f'time per image: {end_time-start_time:.2f} seconds')
        return i_feats
    ...
    def encode_text_s(self, text):
        ...
        S_text_feats,_ = self.STransformer(x,l_aux=0.0, is_text=True)
        S_text_feats = torch.mean(S_text_feats, dim=1)
        t_feats = S_text_feats + t_feats
        end_time = time.time()
        # print(f'time per txt: {end_time - start_time:.2f} seconds')
        return t_feats
